...
class TT(object):
    ...
    *
    ...
    def squeeze(self) -> 'TT':
        ...
        return t_squeeze
    ##################################################
    def ortho_righttwo(self, start_index: Optional[int]=None,
                    end_index: int=1,
                    threshold: float=0,
                    max_rank: int=np.infty) -> 'TT':
    ...
    def svdtwo(self,
        ...
        # left-orthonormalize cores
        if ortho_l is True:
            t = t.ortho_left(end_index=index - 2, threshold=threshold, max_rank=max_rank)

        # right-orthonormalize cores
        if ortho_r is True:
            t = t.ortho_righttwo(end_index=1, threshold=threshold, max_rank=max_rank)

        # decompose 0th core
        [u, s, v] = linalg.svd(t.cores[0].reshape(t.ranks[0] * t.row_dims[0], t.ranks[1]),
                               full_matrices=False, overwrite_a=True, check_finite=False, lapack_driver='gesvd')
        ...
        # update 0th core
        t.cores[0] = u.reshape(t.ranks[0], t.row_dims[0], 1, t.ranks[1])

        # update 0th core
        t.cores[1] = np.tensordot(v, t.cores[1], axes=(1, 0))
    ...
    def pinvtwo(self,
        ...
        # compute gloabl SVD of self
        u, s, v = TT.svdtwo(self, index, threshold=threshold, ortho_l=ortho_l, ortho_r=ortho_r, overwrite=overwrite)

        cores = u.cores + v.cores


        # contract s with (index)th core
        cores[1] = np.tensordot(np.diag(np.reciprocal(s)), cores[1], axes=(1, 0))
